...
now = current_date.strftime("%m/%d/%Y")
current_year = current_date.year
current_day = current_date.strftime("%A")
# print (current_date, now, current_year, current_day)
...
@app.get("/")
async def read_root():
    return {"message": "Welcome to the webhook server!"}
    ...
    system_prompt = """
       ...
       Fields to extract:
        ...
        '''
            ...
            - date_of_service_requested: The date agreed in this conversation if the customer has requested service. 
              Current date is {now}, and the current year is {current_year}. 
              If service was requested for "today", use the current date ({now}). 
              If any other date was decided for the service, add those many days to the current date ({now}) and use that date, ensuring the year is {current_year}. 
...
The current date is {now},the current day is {current_day} ,and the current year is {current_year}.
...
"""
    ...
    try:
        ...
        if supabase_response.error:
            print(f"Supabase insertion failed: {supabase_response.error}")
